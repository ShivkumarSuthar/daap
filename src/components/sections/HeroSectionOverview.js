...
import { HiArrowDown, HiArrowLongRight } from "react-icons/hi2";
...
export default function Home() {
  return (
    <main className="main-wrapper container">
        ...
        <div className="hero__container">
          <div className="hero__heading__wrapper">
            <p className="hero__heading__text">
              Unlock The Potential of Your Brand, Where Strategy<br /> Meets
              Ingenuity
      ...
      <section className="portfolio-section container-fluid py-5">
  <div className="row g-4">
    {COMPANY_INFO.graphics.map((item) => (
      <div key={item.id} className="col-12 col-md-6 col-lg-4">
        <div className="portfolio-card h-100 p-3 shadow-sm rounded">
          <Image 
            src={item.image} 
            alt={item.title} 
            width={450} 
          ...
          />
          <p className="portfolio-title mt-3 fw-bold">{item.title}</p>
        </div>
      </div>
    ))}
    
    <div className="col-12 col-md-6 col-lg-4">
      <div className="portfolio-info-card h-100 d-flex flex-column justify-content-center p-4 shadow-sm rounded">
        <p className="mb-4">We are dedicated to fostering inspiration and reaching milestones through our creative team</p>
        <div className="text-center mt-3">
          <HiArrowNarrowDown className="down-arrow-icon" size={24} />
        </div>
        <div className="text-center mt-3">
          <Link href="/portfolio">
            <span className="btn btn-primary btn__see__portfolio">See Portfolio</span>
          </Link>
        </div>
      </div>
    </div>
